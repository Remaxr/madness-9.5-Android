...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Sad!', 0.2], // From 0% to 19%
		['Horrible', 0.4], // From 20% to 39%
		['Failure', 0.5], // From 40% to 49%
		['Boring', 0.6], // From 50% to 59%
		['Alright', 0.69], // From 60% to 68%
		['Sick!', 0.7], // 69%
		['Rad!', 0.8], // From 70% to 79%
		['Sick!', 0.9], // From 80% to 89%
		['MAD!!!', 1], // From 90% to 99%
		['MAD!!!', 1] // The value on this one isn't used actually, since Perfect is always "1"
	];
				...
				switch (swagCounter)
				{
					...
					case 3:
						if (!skipCountdown)
						{
							...
							FlxG.sound.play(Paths.sound('introGo'/* + '.ogg'+ introSoundsSuffix*/), 1);
							FlxG.sound.play(Paths.sound('goe'));
						//	FlxG.sound.play(Paths.sound('introGo'), 1, false, null, true, function();
						}
					case 4:
				}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			// Std.parseFloat(value1);
			case 'Retainer Damage':
				FlxG.camera.flash(FlxColor.RED, 1.2);
				FlxG.sound.play(Paths.sound('spoopy'));
			...
			case 'retainerfade':
				...
				*/
				...
				FlxG.camera.fade(FlxColor.BLACK, 6.8, true);

			case 'introtext1':
				var retainintro1:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('island/introtext'));
				retainintro1.setGraphicSize(Std.int(retainintro1.width * 1));
				retainintro1.screenCenter();
				retainintro1.antialiasing = true;
				retainintro1.scrollFactor.set(0, 0);
				retainintro1.active = false;
				add(retainintro1);
				FlxG.camera.flash(FlxColor.BLACK, 1.3);

				retainintro1.cameras = [camHUD];
			...
			case 'introtext2':
				var retainintro2:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('island/introtextbutpog'));
				retainintro2.setGraphicSize(Std.int(retainintro2.width * 1));
				retainintro2.screenCenter();
				retainintro2.antialiasing = true;
				retainintro2.scrollFactor.set(0, 0);
				retainintro2.active = false;
				add(retainintro2);
				FlxG.camera.flash(FlxColor.BLACK, 1.3);

				retainintro2.cameras = [camHUD];
